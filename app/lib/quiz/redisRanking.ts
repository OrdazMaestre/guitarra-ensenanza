...
// llega a ninguna insignia); HORNET = mínimo de bronce; ORDAZ = mínimo de plata; MAIkael = máximo
// posible (oro). El "tiempo por respuesta" de cada uno es solo el tiempoSeg que se les asigna para
...
const GHOST_PROFILES: { computeStats: (q: number) => { puntos: number; tiempoSeg: number }; nombre: string }[] = [
  ...
  { nombre: 'HORNET', computeStats: (q) => ({ puntos: q, tiempoSeg: 15 * q }) },
